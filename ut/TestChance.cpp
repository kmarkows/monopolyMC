...
TEST_F(TestChance, TestGoToTile12FromTile6AndTileIsAlreadyOwnedSoPlayerPays10TimesTheRollCardId3) {
  ...
  EXPECT_EQ(game.getPlayersDataForManipulation().at(secondPlayerId - 1).getCurrentBalance(), startingBalance + 70);
}
...
TEST_F(TestChance, TestPayEveryPlayer50CardId13) {
  ...
  EXPECT_EQ(game.getPlayersData().at(1).getCurrentBalance(),
            startingBalance + 50);
  EXPECT_EQ(game.getPlayersData().at(2).getCurrentBalance(),
            startingBalance + 50);
}
